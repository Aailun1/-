...
{
    u16 i;
    u16 coils_start;           // 线圈开始地址
    u16 coils_num;             // 请求的线圈数量
    u8 coils_status[125];      // 最多可读125个线圈的数量
    u8 coils_byte_count = 0;   // 字节计数
    u16 arr_start = 3;         // 响应数据开始位置
    u16 crc;                   // CRC校验结果
    ...
    // 获取线圈开始地址和数量
    coils_start = USART_RX_BUF[2] | (((u16)USART_RX_BUF[1]) << 8);
    coils_num = USART_RX_BUF[4] | (((u16)USART_RX_BUF[3]) << 8);
    ...
    // 计算字节计数，每8位代表一个线圈的状态
    coils_byte_count = (coils_num + 7) / 8;

    // 设置字节计数
    modbus_Tx_buff[2] = coils_byte_count;

    // 遍历虚拟寄存器，获取线圈状态并填充到响应数据中
    for (i = 0; i < coils_num; i++) {
        u8 bit_pos = i % 8;
        u8 byte_pos = i / 8;
        u8 coil_status = modbus_virtual_register[coils_start + i] & 0x01; // ?????????????

        // 复制线圈状态到响应数据的相应位
        coils_status[byte_pos] |= (coil_status << bit_pos);
    }

    // 复制线圈状态数据响应缓存区
    for (i = 0; i < coils_byte_count; i++) {
        modbus_Tx_buff[arr_start++] = coils_status[i];
    }

    // 计算CRC校验值
    crc = CRC16(modbus_Tx_buff, arr_start);

    // 添加CRC到响应数据
    modbus_Tx_buff[arr_start++] = crc & 0xFF;
    modbus_Tx_buff[arr_start++] = (crc >> 8) & 0xFF;

    // 发送响应数据
    modbus_send_data(modbus_Tx_buff, arr_start);
}
...
{
    u16 coil_addr; //主机想要修改从机的指定线圈
    u16 new_state;
    u16 crc;       

    // 将接收到的数据中的线圈地址转换为整数
    coil_addr = USART_RX_BUF[3] | (((u16)USART_RX_BUF[2]) << 8);
    
    //确定线圈状态
    new_state = USART_RX_BUF[5] | (((u16)USART_RX_BUF[4]) << 8);
    
    // 更新虚拟寄存器中对应线圈的状态
    if(new_state == 0xFF00) // 如果激活状态
        modbus_virtual_register[coil_addr] = 1; // 设置线圈为开ON
    else
        modbus_virtual_register[coil_addr] = 0; // 设置线圈为关OFF
    ...
    modbus_Tx_buff[4] = (new_state >> 8) & 0xFF;
    modbus_Tx_buff[5] = new_state & 0xFF;
    
    // 计算CRC校验并加入到响应事件中
    crc = CRC16(modbus_Tx_buff, 6);
    modbus_Tx_buff[6] = crc & 0xFF;
    modbus_Tx_buff[7] = (crc >> 8) & 0xFF;
